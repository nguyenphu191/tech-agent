    ...
    <div className="group bg-gradient-to-b from-white/[0.07] to-white/[0.02] border border-white/10 hover:border-cyan-500/40 rounded-xl overflow-hidden transition-all duration-300 hover:shadow-[0_0_30px_rgba(6,182,212,0.08)]">
      <div className="p-6">
        <div className="flex items-start justify-between mb-4">
          ...
          {product.badge && (
            <span className="px-3 py-1 bg-gradient-to-r from-cyan-500/20 to-blue-500/20 text-cyan-400 text-xs font-bold rounded-full border border-cyan-500/30">
              {product.badge}
            </span>
          )}
        </div>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-white/10">
          ...
          <button
            ...
            className="px-5 py-2 bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white rounded-lg text-sm font-semibold transition-all"
          >
            View Details
          </button>
        </div>
    ...
    <main className="min-h-screen bg-gradient-to-b from-slate-900 via-blue-950 to-slate-900">
      <header className="sticky top-0 z-40 border-b border-white/10 bg-slate-900/80 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-xs font-bold text-cyan-400 tracking-widest uppercase">Enterprise Solutions</span>
          </div>
          <h1 className="text-4xl sm:text-5xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-300 via-cyan-300 to-blue-300">
            Enterprise Platform
          </h1>
          <p className="text-gray-400 mt-2 text-sm">Scalable cloud infrastructure for mission-critical applications</p>
        </div>
      </header>

      <div className="border-b border-white/10 bg-gradient-to-r from-blue-500/5 to-cyan-500/5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {stats.map(({ icon: Icon, label, value }) => (
              <div key={label} className="group bg-white/5 backdrop-blur rounded-lg p-4 border border-white/10 hover:border-cyan-500/30 transition-all">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="flex flex-col sm:flex-row gap-4 mb-8">
          ...
          <button
            onClick={handleReset}
            className="px-6 py-2 rounded-lg border border-white/20 text-white hover:border-cyan-500/30 hover:bg-white/5 transition-all text-sm font-medium"
          >
            Reset Filters
          </button>
        </div>
        ...
        <div className="flex gap-8">
          <div className="w-56 flex-shrink-0 max-h-fit sticky top-32">
            <div className="bg-gradient-to-b from-white/5 to-white/[0.02] backdrop-blur rounded-xl border border-white/10 p-6 space-y-6">
            ...
            {filteredProducts.length > 0 ? (
              <>
                ...
                <div className="mt-8 text-center text-xs text-gray-500">
                  Showing {filteredProducts.length} of {CORPORATE_PRODUCTS.length} solutions
                </div>
              </>
            ) : (
              <div className="text-center py-20">
                <p className="text-gray-400 text-lg mb-4">No solutions match your criteria</p>
                <button
                  onClick={handleReset}
      ...
      {selectedProduct && (
        ...
        >
          <div
            className="bg-gradient-to-b from-slate-800 to-slate-900 rounded-xl p-8 max-w-2xl w-full border border-white/10 max-h-[80vh] overflow-y-auto"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex justify-between items-start mb-6">
              <div>
                <h3 className="text-3xl font-bold text-white">{selectedProduct.name}</h3>
                <p className="text-cyan-400 text-sm mt-2 font-medium">{selectedProduct.category}</p>
              </div>
              <button
                onClick={() => setSelectedProduct(null)}
            ...
            <div className="bg-white/5 backdrop-blur rounded-lg p-6 mb-8 border border-white/10">
              <p className="text-xs text-gray-400 font-bold uppercase tracking-wider mb-4">Technical Specifications</p>
              <p className="text-white text-sm leading-relaxed">
                {selectedProduct.metadata?.specs || selectedProduct.description}
              </p>
            </div>

            <div className="flex gap-3">
              <button className="flex-1 bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 text-white py-3 rounded-lg font-semibold transition-all">
                Deploy Solution
              </button>
              <button
                onClick={() => setSelectedProduct(null)}
